...
interface IV51MetadataRouter {
    function tokenURI(uint256 tokenId) external view returns (string memory);
}

interface IV51VaultDestinations {
...
contract TrollInHoodGenesisV51 is ERC721Enumerable, ERC2981, Ownable2Step, IERC4906, ReentrancyGuard {
    enum Family {
    ...
    IV51VaultDestinations public vaultFactory;
    event VaultFactoryConfigured(address indexed factory);
    ...
    /// @notice Wire once, before minting; production must verify factory bytecode.
    function setVaultFactory(address factory_) external onlyOwner {
        require(address(vaultFactory) == address(0), "factory frozen");
        require(factory_ != address(0), "zero factory");
        require(IV51VaultDestinations(factory_).nftCollection() == address(this), "wrong collection");
        vaultFactory = IV51VaultDestinations(factory_);
        emit VaultFactoryConfigured(factory_);
    }
        ...
        returns (uint256 firstTokenId)
    {
        ...
        require(address(vaultFactory) != address(0), "factory not configured");
        require(to != address(0), "zero recipient");
        require(quantity > 0, "zero quantity");
        if (nextTokenId + quantity - 1 > TrollV51Config.MAX_SUPPLY) revert SoldOut();
    ...
    function revealFamily(uint256 tokenId, Family f, bytes32[] calldata proof) external {
        ...
        if (!Sha256MerkleV51.verify(proof, TrollV51Config.FAMILY_MERKLE_ROOT, leaf)) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IV51MetadataRouter(metadataRouter).tokenURI(tokenId);
    }

    function genesisManifestSHA256() external pure returns (bytes32) {
        return TrollV51Config.GENESIS_MANIFEST_SHA256;
    }

    function familyMerkleRoot() external pure returns (bytes32) {
        return TrollV51Config.FAMILY_MERKLE_ROOT;
    }

    function maxSupply() external pure returns (uint256) {
        return TrollV51Config.MAX_SUPPLY;
    }
    ...
    {
        if (to != address(0) && address(vaultFactory) != address(0)) {
            require(!vaultFactory.isVault(to), "passive vault destination");
            require(to != vaultFactory.predictVault(tokenId), "passive vault destination");
        }
...
contract TrollBoundVaultV51 is IERC721Receiver, IERC1155Receiver {
    ...
    function onERC721Received(address, address, uint256, bytes calldata)
        ...
        returns (bytes4)
    {
        require(msg.sender != nftCollection, "genesis deposit forbidden");
...
contract TrollBoundVaultFactoryV51 {
    address public immutable nftCollection;
    ...
    function createVault(uint256 tokenId) external returns (address vault) {
        require(IV51NFTForEvolution(nftCollection).ownerOf(tokenId) != address(0), "nonexistent NFT");
        require(vaultOf[tokenId] == address(0), "exists");
        bytes32 salt = keccak256(abi.encode(nftCollection, tokenId));
        vault = address(new TrollBoundVaultV51{salt: salt}(nftCollection, tokenId, address(this)));
    ...
    function predictVault(uint256 tokenId) external view returns (address predicted) {
        ...
        bytes memory code = abi.encodePacked(
            type(TrollBoundVaultV51).creationCode,
...
interface IV51NFTForEvolution {
    function ownerOf(uint256 tokenId) external view returns (address);
    function notifyMetadataUpdate(uint256 tokenId) external;
}

contract TrollEvolutionEngineV51 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    enum State { GENESIS, AWAKENED, CHARGED, RELIC, ASCENDED, LEGENDARY, OMEGA }

    IERC20 public immutable troll;
    IV51NFTForEvolution public immutable nft;
    ...
    function stateOf(uint256 tokenId) public view returns (State) {
        ...
        if (b >= TrollV51Config.OMEGA) return State.OMEGA;
        if (b >= TrollV51Config.LEGENDARY) return State.LEGENDARY;
        if (b >= TrollV51Config.ASCENDED) return State.ASCENDED;
        if (b >= TrollV51Config.RELIC) return State.RELIC;
        if (b >= TrollV51Config.CHARGED) return State.CHARGED;
        if (b >= TrollV51Config.AWAKENED) return State.AWAKENED;
...
interface IV51StateReader {
    function stateOf(uint256 tokenId) external view returns (TrollEvolutionEngineV51.State);
}

contract TrollMetadataRouterV51 {
    using Strings for uint256;
    string public baseURI;
    IV51StateReader public immutable evolution;

    constructor(string memory baseURI_, address evolution_) {
        require(bytes(baseURI_).length > 0 && evolution_ != address(0), "bad config");
        baseURI = baseURI_;
        evolution = IV51StateReader(evolution_);
    }
...
contract TrollAssetRegistryV51 is Ownable2Step {
    ...
    function _registerAsset(
        ...
        require(_asset[assetKey].token == address(0), "asset identity frozen");
    ...
    /// @notice Emergency enable/disable does not reinterpret historical asset identity.
    function setAssetEnabled(bytes32 assetKey, bool enabled) external onlyOwner {
        require(_asset[assetKey].token != address(0), "unknown asset");
        _asset[assetKey].enabled = enabled;
        emit AssetEnabled(assetKey, enabled);
    }
...
interface IV51VaultFactory {
    function nftCollection() external view returns (address);
    function vaultOf(uint256 tokenId) external view returns (address);
}

interface IV51AssetRegistry {
    function asset(bytes32 assetKey)
        external
        view
        returns (TrollAssetRegistryV51.AssetConfig memory);
}

contract TrollRewardRouterV51 is Ownable2Step, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IV51VaultFactory public immutable vaultFactory;
    IV51AssetRegistry public immutable registry;
    IV51NFTForEvolution public immutable nft;
    ...
    constructor(address initialOwner, address vaultFactory_, address registry_) Ownable(initialOwner) {
        ...
        vaultFactory = IV51VaultFactory(vaultFactory_);
        registry = IV51AssetRegistry(registry_);
        nft = IV51NFTForEvolution(IV51VaultFactory(vaultFactory_).nftCollection());
    }
    ...
    function depositDirect(uint256 tokenId, bytes32 assetKey, uint256 amount)
        ...
        nonReentrant
    {
        require(nft.ownerOf(tokenId) != address(0), "nonexistent NFT");
        require(amount > 0, "zero amount");
        TrollAssetRegistryV51.AssetConfig memory a = registry.asset(assetKey);
        require(a.enabled, "asset disabled");
        require(a.chainId == block.chainid, "wrong chain");
        require(a.lane == TrollAssetRegistryV51.Lane.DIRECT_VAULT, "regulated lane");
    ...
    function recordRegulatedEntitlement(
        ...
        require(a.enabled, "asset disabled");
        require(a.chainId == block.chainid, "wrong chain");
        require(a.lane == TrollAssetRegistryV51.Lane.REGULATED_ENTITLEMENT, "direct lane");
...
interface IV51EvolutionForLens {
...
contract TrollLensV51 {
    ...
    IV51GenesisForLens public immutable nft;
    IV51EvolutionForLens public immutable evolution;
    IV51VaultFactory public immutable vaultFactory;
    ...
    constructor(address nft_, address evolution_, address vaultFactory_) {
        ...
        nft = IV51GenesisForLens(nft_);
        evolution = IV51EvolutionForLens(evolution_);
        vaultFactory = IV51VaultFactory(vaultFactory_);
    }
...
contract TrollAssetSnapshotAnchorV51 is Ownable2Step {
    struct Snapshot {
        bytes32 manifestHash;
        uint64 blockNumber;
        uint64 timestamp;
    ...
    constructor(address initialOwner, address nft_) Ownable(initialOwner) {
        require(initialOwner != address(0) && nft_ != address(0), "zero");
        nft = IV51NFTForEvolution(nft_);
    }
...
contract MockAssetV51 is ERC20 {
...
/// @dev Used by V51 security tests to verify actual-credit accounting.
contract FeeOnTransferAssetV51 is ERC20 {
...
contract ReentrantMintReceiverV51 is IERC721Receiver {
    TrollInHoodGenesisV51 public immutable nft;
    bool public attempted;
    bool public reentrySucceeded;
    uint256 public lastFirstTokenId;

    constructor(address nft_) {
        nft = TrollInHoodGenesisV51(nft_);
    }
