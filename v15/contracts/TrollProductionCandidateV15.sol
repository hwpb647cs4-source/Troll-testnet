...
interface IV15MetadataRouter {
    function tokenURI(uint256 tokenId) external view returns (string memory);
}

contract TrollInHoodGenesisV15 is ERC721Enumerable, ERC2981, Ownable2Step, IERC4906 {
    enum Family {
        ...
        returns (uint256 firstTokenId)
    {
        ...
        if (nextTokenId + quantity - 1 > TrollV15Config.MAX_SUPPLY) revert SoldOut();
    ...
    function revealFamily(uint256 tokenId, Family f, bytes32[] calldata proof) external {
        ...
        if (!Sha256MerkleV15.verify(proof, TrollV15Config.FAMILY_MERKLE_ROOT, leaf)) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IV15MetadataRouter(metadataRouter).tokenURI(tokenId);
    }

    function genesisManifestSHA256() external pure returns (bytes32) {
        return TrollV15Config.GENESIS_MANIFEST_SHA256;
    }

    function familyMerkleRoot() external pure returns (bytes32) {
        return TrollV15Config.FAMILY_MERKLE_ROOT;
    }

    function maxSupply() external pure returns (uint256) {
        return TrollV15Config.MAX_SUPPLY;
    }
...
contract TrollBoundVaultV15 is IERC721Receiver, IERC1155Receiver {
...
contract TrollBoundVaultFactoryV15 {
    address public immutable nftCollection;
    ...
    function createVault(uint256 tokenId) external returns (address vault) {
        ...
        vault = address(new TrollBoundVaultV15{salt: salt}(nftCollection, tokenId, address(this)));
    ...
    function predictVault(uint256 tokenId) external view returns (address predicted) {
        ...
        bytes memory code = abi.encodePacked(
            type(TrollBoundVaultV15).creationCode,
...
interface IV15NFTForEvolution {
    function ownerOf(uint256 tokenId) external view returns (address);
    function notifyMetadataUpdate(uint256 tokenId) external;
}

contract TrollEvolutionEngineV15 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    enum State { GENESIS, AWAKENED, CHARGED, RELIC, ASCENDED, LEGENDARY, OMEGA }

    IERC20 public immutable troll;
    IV15NFTForEvolution public immutable nft;
    ...
    function stateOf(uint256 tokenId) public view returns (State) {
        ...
        if (b >= TrollV15Config.OMEGA) return State.OMEGA;
        if (b >= TrollV15Config.LEGENDARY) return State.LEGENDARY;
        if (b >= TrollV15Config.ASCENDED) return State.ASCENDED;
        if (b >= TrollV15Config.RELIC) return State.RELIC;
        if (b >= TrollV15Config.CHARGED) return State.CHARGED;
        if (b >= TrollV15Config.AWAKENED) return State.AWAKENED;
...
interface IV15StateReader {
    function stateOf(uint256 tokenId) external view returns (TrollEvolutionEngineV15.State);
}

contract TrollMetadataRouterV15 {
    using Strings for uint256;
    string public baseURI;
    IV15StateReader public immutable evolution;

    constructor(string memory baseURI_, address evolution_) {
        require(bytes(baseURI_).length > 0 && evolution_ != address(0), "bad config");
        baseURI = baseURI_;
        evolution = IV15StateReader(evolution_);
    }
...
contract TrollAssetRegistryV15 is Ownable2Step {
...
interface IV15VaultFactory {
    function vaultOf(uint256 tokenId) external view returns (address);
}

interface IV15AssetRegistry {
    function asset(bytes32 assetKey)
        external
        view
        returns (TrollAssetRegistryV15.AssetConfig memory);
}

contract TrollRewardRouterV15 is Ownable2Step, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IV15VaultFactory public immutable vaultFactory;
    IV15AssetRegistry public immutable registry;
    ...
    constructor(address initialOwner, address vaultFactory_, address registry_) Ownable(initialOwner) {
        ...
        vaultFactory = IV15VaultFactory(vaultFactory_);
        registry = IV15AssetRegistry(registry_);
    }
    ...
    {
        require(amount > 0, "zero amount");
        TrollAssetRegistryV15.AssetConfig memory a = registry.asset(assetKey);
        require(a.enabled, "asset disabled");
        require(a.chainId == block.chainid, "wrong chain");
        require(a.lane == TrollAssetRegistryV15.Lane.DIRECT_VAULT, "regulated lane");
    ...
    function recordRegulatedEntitlement(
        ...
        TrollAssetRegistryV15.AssetConfig memory a = registry.asset(assetKey);
        require(a.enabled, "asset disabled");
        require(a.lane == TrollAssetRegistryV15.Lane.REGULATED_ENTITLEMENT, "direct lane");
...
interface IV15EvolutionForLens {
...
contract TrollLensV15 {
    ...
    IV15GenesisForLens public immutable nft;
    IV15EvolutionForLens public immutable evolution;
    IV15VaultFactory public immutable vaultFactory;
    ...
    constructor(address nft_, address evolution_, address vaultFactory_) {
        ...
        nft = IV15GenesisForLens(nft_);
        evolution = IV15EvolutionForLens(evolution_);
        vaultFactory = IV15VaultFactory(vaultFactory_);
    }
...
contract TrollAssetSnapshotAnchorV15 is Ownable2Step {
    struct Snapshot {
        bytes32 manifestHash;
        uint64 blockNumber;
        uint64 timestamp;
...
contract MockAssetV15 is ERC20 {
...
/// @dev Used by V15 security tests to verify actual-credit accounting.
contract FeeOnTransferAssetV15 is ERC20 {
