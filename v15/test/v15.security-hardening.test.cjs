...
describe("TROLL NFT 2.0 V15 security hardening", function () {
  this.timeout(240000);

  it("preserves the V14 proven lifecycle while hardening asset accounting", async function () {
    const [walletA, walletB, stranger, feeSink] = await ethers.getSigners();
    const chainId = Number((await ethers.provider.getNetwork()).chainId);

    const troll = await deploy("MockAssetV15", "Test TROLL", "tTROLL", 18);
    const stable = await deploy("MockAssetV15", "Test USD", "tUSD", 6);
    const gold = await deploy("MockAssetV15", "Test Gold", "tGOLD", 18);
    const silver = await deploy("MockAssetV15", "Test Silver", "tSILVER", 18);
    const stock = await deploy("MockAssetV15", "Test NVIDIA", "tNVDA", 18);
    const feeAsset = await deploy("FeeOnTransferAssetV15", "Fee Asset", "FEE", 1000, feeSink.address);

    const nft = await deploy("TrollInHoodGenesisV15", walletA.address, walletA.address, 500);
    const evolution = await deploy("TrollEvolutionEngineV15", await troll.getAddress(), await nft.getAddress(), DEAD);
    const factory = await deploy("TrollBoundVaultFactoryV15", await nft.getAddress());
    const metadata = await deploy("TrollMetadataRouterV15", "ipfs://troll-v13/", await evolution.getAddress());
    const registry = await deploy("TrollAssetRegistryV15", walletA.address);
    const router = await deploy("TrollRewardRouterV15", walletA.address, await factory.getAddress(), await registry.getAddress());
    const lens = await deploy("TrollLensV15", await nft.getAddress(), await evolution.getAddress(), await factory.getAddress());
    const snapshots = await deploy("TrollAssetSnapshotAnchorV15", walletA.address);
    ...
    const wrongChainToken = await deploy("MockAssetV15", "Wrong Chain", "WRONG", 18);
    ...
    const entitlementHash = ethers.id("TROLL-V15-NVDA-ENTITLEMENT-1");
    ...
    const snapshotHash = ethers.id("TROLL-V15-ASSET-SNAPSHOT-1");
