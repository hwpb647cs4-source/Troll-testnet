...
describe("TROLL NFT 2.0 V19 Slither remediation", function () {
  this.timeout(240000);

  it("rejects native ETH sent directly to the passive vault", async function () {
    const [walletA] = await ethers.getSigners();
    const nft = await deploy("TrollInHoodGenesisV19", walletA.address, walletA.address, 500);
    const factory = await deploy("TrollBoundVaultFactoryV19", await nft.getAddress());
  ...
  it("blocks safeMint callback reentrancy and reserves state before callbacks", async function () {
    ...
    const nft = await deploy("TrollInHoodGenesisV19", walletA.address, walletA.address, 500);
    const receiver = await deploy("ReentrantMintReceiverV19", await nft.getAddress());
  ...
  it("preserves the proven lifecycle after Slither remediation", async function () {
    ...
    const troll = await deploy("MockAssetV19", "Test TROLL", "tTROLL", 18);
    const stable = await deploy("MockAssetV19", "Test USD", "tUSD", 6);
    const gold = await deploy("MockAssetV19", "Test Gold", "tGOLD", 18);
    const silver = await deploy("MockAssetV19", "Test Silver", "tSILVER", 18);
    const stock = await deploy("MockAssetV19", "Test NVIDIA", "tNVDA", 18);
    const feeAsset = await deploy("FeeOnTransferAssetV19", "Fee Asset", "FEE", 1000, feeSink.address);

    const nft = await deploy("TrollInHoodGenesisV19", walletA.address, walletA.address, 500);
    const evolution = await deploy("TrollEvolutionEngineV19", await troll.getAddress(), await nft.getAddress(), DEAD);
    const factory = await deploy("TrollBoundVaultFactoryV19", await nft.getAddress());
    const metadata = await deploy("TrollMetadataRouterV19", "ipfs://troll-v13/", await evolution.getAddress());
    const registry = await deploy("TrollAssetRegistryV19", walletA.address);
    const router = await deploy("TrollRewardRouterV19", walletA.address, await factory.getAddress(), await registry.getAddress());
    const lens = await deploy("TrollLensV19", await nft.getAddress(), await evolution.getAddress(), await factory.getAddress());
    const snapshots = await deploy("TrollAssetSnapshotAnchorV19", walletA.address);
    ...
    const wrongChainToken = await deploy("MockAssetV19", "Wrong Chain", "WRONG", 18);
    ...
    const entitlementHash = ethers.id("TROLL-V19-NVDA-ENTITLEMENT-1");
    ...
    const snapshotHash = ethers.id("TROLL-V19-ASSET-SNAPSHOT-1");
